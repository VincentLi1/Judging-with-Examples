...
log_path = parent_dir + '/bias/Dstraction_Bias/Dstraction_Bias(g.t)_A.json'
...
def generate_prompt(questions, chosen, rejected, position_choices, number, answers):
    ...
    for i in range(len(questions)):
        ...
        if position_choices[i] == 1:
            prompts.append(Dstraction_Bias_A(question=questions[i], answer_a=chosen[i], answer_b=rejected[i]))
        else:
            prompts.append(Dstraction_Bias_A(question=questions[i], answer_a=rejected[i], answer_b=chosen[i]))
    return prompts
